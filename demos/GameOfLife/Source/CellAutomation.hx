...
class CellAutomation
{
	...
	public static function nextLifeGeneration(src_img:Image, dest_img:Image, rule:String, bgColor:Int, fgColor:Int, swap:Bool = false):Void
	{
		if (swap)
		{
			nextLifeGeneration(dest_img, src_img, rule, bgColor, fgColor);
			return;
		}
		...
		for (c in rule.split('/')[0].split(''))
			s_rule |= 1 << Std.parseInt(c);
		for (c in rule.split('/')[1].split(''))
			b_rule |= 1 << Std.parseInt(c);
		...
		for (y in 0...h)
		{
			...
			for (x in 0...w)
			{
				...
				{
					if ((s_rule & sum) > 0) dest_img.setPixel32(x, y, fgColor); // life again
					else
						dest_img.setPixel32(x, y, bgColor); // dead
				}
				else // old cell is dead
				{
					if ((b_rule & sum) > 0) dest_img.setPixel32(x, y, fgColor); // birthday
					else
